...
class BeliefPropagation(Inference):
    ...
    def __init__(self, model):
        ...
        self.working_junction_tree = copy.deepcopy(self.junction_tree)

    def _query2(self, variables, operation, evidence=None):

        if self.working_junction_tree.evidence != evidence:
            self.working_junction_tree = copy.deepcopy(self.junction_tree)
            self.working_junction_tree.add_evidence(evidence)

        if not self.working_junction_tree.is_calibrated:
            self.working_junction_tree.calibrate()

        results = dict()
        for v in variables:
            for node in self.working_junction_tree.nodes():
                if v in node:
                    results[v] = self.working_junction_tree.clique_beliefs[node].marginalize(list(set(self.working_junction_tree.clique_beliefs[node].variables) - set([v])), inplace=False).normalize(inplace=False)
                    break
    ...
    def _query(self, variables, operation, evidence=None):
        ...
        """
        junction_tree = copy.deepcopy(self.junction_tree)

        junction_tree.calibrate()
        ...
        for var in query_variables:
            nodes_with_query_variables.update(filter(lambda x: var in x, junction_tree.nodes()))
        ...
        for i in range(len(nodes_with_query_variables) - 1):
            subtree_nodes.update(nx.shortest_path(junction_tree, nodes_with_query_variables[i],
                                                  nodes_with_query_variables[i + 1]))
        subtree_undirected_graph = junction_tree.subgraph(subtree_nodes)
        ...
        clique_potential_list = [junction_tree.clique_beliefs[root_node]]
        ...
        while parent_nodes:
            ...
            for child_node in set(subtree.neighbors(parent_node)) - nodes_traversed:
                clique_potential_list.append(junction_tree.clique_beliefs[child_node] /
                                             junction_tree.sepset_beliefs[frozenset([parent_node, child_node])])
        ...
        if operation == 'marginalize':
            result = variable_elimination.query(variables=variables, evidence=evidence)
            return result
    ...
    def query2(self, variables, evidence=None):
        """
        ...
        """
        return self._query2(variables=variables, operation='marginalize', evidence=evidence)
